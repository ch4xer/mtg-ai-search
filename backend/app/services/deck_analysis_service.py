...
from ..llm_provider import get_chat_provider_config, is_chat_provider_configured
...
async def analyze_owned_deck(deck_id: str, user_id: str) -> dict:
    if not is_chat_provider_configured():
        provider = get_chat_provider_config().provider
        raise HTTPException(status_code=503, detail=f"{provider} API key not configured")
